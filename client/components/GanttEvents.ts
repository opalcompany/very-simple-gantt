...
export interface onGanttDragBarEvent { (bar: GanttBar, newStartTime: Date, bars: GanttBar[]): boolean }
...
// must return false if resizing is not allowed for the bar, true if allowed
export interface onGanttResizeBarEvent { (resizedBar: GanttBar, newEndTime: Date, bars: GanttBar[]): boolean }
